...
struct Queue : public Object<QueueType> {
    ...
    /* Previously, 'return dispatch_get_main_queue();' was used here, but for
     some reason clang emits objc_retainAutorelease() function calls in this case,
     and at the same time if dispatch_get_main_queue() called directly clang
     doesn't generate any additional code. This default argument hack seems to 
     force clang not to generate any unnecessary code. */
